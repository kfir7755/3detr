...
#         device = xyz.device
#         global_min = torch.tensor([-1.0, -1.0, -1.0], device=device).unsqueeze(0) # Shape (1, 3)
#         global_max = torch.tensor([ 1.0,  1.0,  1.0], device=device).unsqueeze(0)

#         # 2. Use these fixed dims instead of batch-specific dims
#         point_cloud_dims = [global_min, global_max] 
...
class Landmark3DETR(nn.Module):
    def __init__(self, pre_encoder, encoder, decoder, num_landmarks=17, embed_dim=256):
        ...
        # Learnable embeddings for the 17 distinct landmarks
        # These replace the FPS-sampled queries from the original 3DETR
        # Shape (17, 3) initialized near the origin or mean landmark positions
        self.query_points = nn.Parameter(torch.randn(num_landmarks, 3) * 0.1) 
        
        # Keep the content embedding (optional, but good for feature extraction)
        self.query_content = nn.Embedding(num_landmarks, embed_dim)
    ...
    def forward(self, inputs):
        ...
        # Calculate positional embeddings for the points based on XYZ
        device = xyz.device
        global_min = torch.tensor([[-1.0, -1.0, -1.0]] * batch_size, device=device)  # Shape (B, 3)
        global_max = torch.tensor([[1.0,  1.0,  1.0]] * batch_size, device=device)  # Shape (B, 3)
        ...
        query_pos_embed = self.pos_embedding(
            self.query_points.unsqueeze(0).repeat(batch_size, 1, 1), # (B, 17, 3)
            input_range=point_cloud_dims
        )
        query_pos_embed = query_pos_embed.permute(2, 0, 1) # (C, B, 17) -> (17, B, C) if needed by decoder

        # Prepare Decoder Inputs
        tgt = self.query_content.weight.unsqueeze(1).repeat(1, batch_size, 1) # (17, B, C)
        
        # CHANGE 3: Pass the explicitly computed PE
        decoder_output = self.decoder(
            tgt, memory, query_pos=query_pos_embed, pos=pos
        )[0][-1]

        # CHANGE 4: Predict OFFSETS, not absolute coordinates
        # The head predicts (B, 17, 3) delta
        delta_coords = self.landmark_head(decoder_output.permute(1, 2, 0)).transpose(1, 2)
        
        # Final prediction = Anchor + Delta
        coords = self.query_points.unsqueeze(0) + delta_coords
